...
docker build -f "$DIR/src/main/docker/Dockerfile.jvm" --no-cache --progress=plain -t caravanacloud/rinhadebackend:latest "$DIR"
docker push caravanacloud/rinhadebackend-jvm:latest

echo "## Building Native image"
docker build -f "$DIR/src/main/docker/Dockerfile.native" --no-cache --progress=plain -t caravanacloud/rinhadebackend:latest "$DIR"
